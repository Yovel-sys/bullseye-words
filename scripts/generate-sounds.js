// Script חד-פעמי ליצירת קובצי WAV קטנים לצלילי המשחק (סינתזה טהורה,
// בלי תלות חיצונית). הרצה: node scripts/generate-sounds.js
const fs = require('fs');
...
// עטיפת אנוולופ (attack/decay) כדי שהצליל לא "יקליק" בגבולות
...
function tone(freqStart, freqEnd, duration, attack, decay) {
  const n = Math.floor(SAMPLE_RATE * duration);
  const out = new Array(n);
  for (let i = 0; i < n; i++) {
    const t = i / SAMPLE_RATE;
    const freq = freqStart + (freqEnd - freqStart) * (t / duration);
    out[i] = Math.sin(2 * Math.PI * freq * t) * envelope(t, duration, attack, decay);
  }
  return out;
}
...
// נקישה קצרצרה להקלדת אות - מושמעת הרבה פעמים ברצף, ולכן שקטה מאוד
const letterClick = mix(
  scale(noise(0.008, 0.0005, 0.006), 0.2),
  scale(tone(2400, 2000, 0.006, 0.0005, 0.005), 0.12)
);
writeWav(path.join(outDir, 'letterClick.wav'), letterClick);

// צליל ניצחון - שלושה תווים עולים (בול פגיעה!)
const correct = concat(
  scale(tone(880, 880, 0.09, 0.005, 0.05), 0.5),
  silence(0.02),
  scale(tone(1108.7, 1108.7, 0.09, 0.005, 0.05), 0.5),
  silence(0.02),
  scale(tone(1318.5, 1318.5, 0.16, 0.005, 0.1), 0.55)
);
writeWav(path.join(outDir, 'correct.wav'), correct);

// צליל שגיאה (מילה לא תקנית) - טון נמוך ויורד, קצר ורך
const incorrect = scale(tone(320, 180, 0.16, 0.006, 0.1), 0.16);
writeWav(path.join(outDir, 'incorrect.wav'), incorrect);

// צליל ניחוש שנקלט (אבל לא פגיעה) - נייטרלי, לא שלילי: שני "טוק" קצרים
const guess = concat(
  scale(tone(600, 600, 0.06, 0.004, 0.045), 0.22),
  silence(0.045),
  scale(tone(600, 600, 0.06, 0.004, 0.045), 0.22)
);
writeWav(path.join(outDir, 'guess.wav'), guess);
